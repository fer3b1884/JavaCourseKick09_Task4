...
public class DeleteTripCommand implements Command {
    private static final Logger logger = LogManager.getLogger(DeleteTripCommand.class);
    ...
    public CommandResult execute(HttpServletRequest request) throws CommandException {
        ...
        if (idParam.isEmpty()) {
            CommandUtil.setFlashError(request, "Trip ID is required");
        ...
        try {
            ...
            if (deleted) {
                CommandUtil.setFlashMessage(request, "Trip deleted successfully");
            } else {
                CommandUtil.setFlashError(request, "Trip not found");
            }
        } catch (NumberFormatException e) {
            logger.error("Invalid trip id: {}", idParam);
            CommandUtil.setFlashError(request, "Invalid trip ID");
        } catch (ServiceException e) {
            logger.error("Failed to delete trip id: {}", idParam, e);
            CommandUtil.setFlashError(request, "Failed to delete trip: " + e.getMessage());
        }
        return CommandResult.redirect(request.getContextPath() + "/controller?command=admin_trips");
    }
}