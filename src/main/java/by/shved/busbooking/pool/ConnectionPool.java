...
public final class ConnectionPool {
    ...
    private static final String POOL_TIMEOUT_PROPERTY = "db.pool.timeout";
    ...
    private ConnectionPool() {
        ...
        connectionTimeoutMillis = getPoolTimeoutMillis(properties);
        if (url == null || username == null || password == null) {
            throw new ExceptionInInitializerError("Database properties are not configured correctly");
        }
        try {
    ...
    private int getPoolSize(Properties properties) {
        ...
        try {
            ...
            throw new IllegalArgumentException("Invalid pool size: " + poolSizeProperty, e);
        }
    }

    private long getPoolTimeoutMillis(Properties properties) {
        String timeoutProperty = properties.getProperty(POOL_TIMEOUT_PROPERTY);
        if (timeoutProperty == null) {
            return TimeUnit.SECONDS.toMillis(DEFAULT_POOL_TIMEOUT_SECONDS);
        }
        try {
            long timeoutSeconds = Long.parseLong(timeoutProperty);
            if (timeoutSeconds <= 0) {
                throw new IllegalArgumentException("Pool timeout must be greater than zero");
            }
            return TimeUnit.SECONDS.toMillis(timeoutSeconds);
        } catch (NumberFormatException e) {
            throw new IllegalArgumentException("Invalid pool timeout: " + timeoutProperty, e);
        }
    }
    ...
    private void initializePool(String url, String username, String password, int poolSize) {
        ...
        try {
            ...
            logger.info("Connection pool initialized. Size: {}, timeout: {} ms", poolSize, connectionTimeoutMillis);
        } catch (SQLException e) {
    ...
    public ProxyConnection getConnection() throws ConnectionPoolException {
        try {
            ProxyConnection connection = freeConnections.poll(connectionTimeoutMillis, TimeUnit.MILLISECONDS);
            if (connection == null) {
                throw new ConnectionPoolException("Timeout waiting for connection from pool (" + connectionTimeoutMillis + " ms)");
            }
            logger.debug("Connection obtained. Free connections: {}", freeConnections.size());
            return connection;
        } catch (InterruptedException e) {
            Thread.currentThread().interrupt();
            throw new ConnectionPoolException("Thread interrupted while obtaining connection from pool", e);
        }
    }
    ...
    public void releaseConnection(ProxyConnection proxyConnection) {
        ...
        try {
            if (!freeConnections.contains(proxyConnection)) {
                freeConnections.put(proxyConnection);
                logger.debug("Connection returned to pool. Free connections: {}", freeConnections.size());
            }
        } catch (InterruptedException e) {
            Thread.currentThread().interrupt();
    ...
    private void closeAllConnections() {
        for (ProxyConnection proxyConnection : allConnections) {
            try {
                proxyConnection.closeRealConnection();
            } catch (SQLException e) {
